...
public class MapleQuestRequirement {
    ...
    boolean check(MapleCharacter c, Integer npcid) {
        switch (getType()) {
            case JOB:
                return c.isGM() || matchesJob(c.getJob());
            case QUEST:
                for (MapleData questEntry : getData().getChildren()) {
                    ...
                    if (q == null && MapleQuestStatus.Status.getById(MapleDataTool.getInt(questEntry.getChildByPath("state"))).orElse(MapleQuestStatus.Status.NOT_STARTED).equals(MapleQuestStatus.Status.NOT_STARTED))
                        continue;
                    if (q == null || !q.getStatus().equals(MapleQuestStatus.Status.getById(MapleDataTool.getInt(questEntry.getChildByPath("state"))).orElse(MapleQuestStatus.Status.NOT_STARTED)))
                        return false;
                }
                return true;
            case ITEM:
...
//			case MIN_PET_TAMENESS:
    ...
    private boolean matchesJob(MapleJob job) {
        return getData().getChildren().stream()
