...
public class UseMountFoodHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.getPlayer().getInventory(MapleInventoryType.USE).findById(itemid).isEmpty()) {
            return;
        }
        ...
        c.getPlayer().getMount().setExperience(expRandom(c.getPlayer().getMount().getLevel()) * ChannelServer.getInstance(c.getChannel()).getMountRate() + c.getPlayer().getMount().getExperience());
        int level = c.getPlayer().getMount().getLevel();
        if (c.getPlayer().getMount().getExperience() >= ExpTable.getMountExpNeededForLevel(level) && level < 31 && c.getPlayer().getMount().getTiredness() != 0) {
